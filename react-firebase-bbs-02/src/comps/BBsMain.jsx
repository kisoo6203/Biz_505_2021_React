...
function BBsMain() {
	...
	const firebaseFetch = async () =>{
		...
		/**
		 * 칼럼으로 정렬하기
		 * collection("").orderBy("칼럼명") 칼럼명으로 오름차순정렬
		 * collection("").orderBy("칼럼명","desc") 내림차순
		 * 
		 * collection.where() 조건지정이 가능
		 * 
		 * 조건 지정을 할 때 부등호가 들어가는 조건을 사용할경우
		 * 반드시 해당 칼럼을 먼저 정렬해줘야 한다
		 * collection.orderBy("b_date")
		 * 		.where('b_date > "2021-09-01" ')
		 * 
		 * orderBy()를 두개 이상의 칼럼에 지정할경우
		 * firestore 콘솔에서 복합색인을 추가 해주어야 한다
		 * 
