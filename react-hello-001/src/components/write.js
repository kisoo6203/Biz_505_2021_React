 ...
 * b_name 이라는 변수에 문자열을 저장하도록 코드가 작동 될 것이다.
 * 
 * 하지만 react 에서는 b_name = 이름과 같은 코드는 금지된다
 * 때문에 react로 form을 만들고 value속성에 변수명을 지정하는 순간
 * input box는 read only가 된다
 * 
 * 그렇다면 input에 뭔가를 입력하고 싶은데
 * read only가 되어버리면 input을 사용한느 의미가 없어진다
...
const Write =() =>{
	...
	const [b_name, setB_Name] = useState("리몽룡");
	 ...
	 * 전달된 키보드 문자열은 b_writer_text 변수에 임시 담고
	 ...
	 * 2. 실제 b_name 변수가 가리키고있는 메모리의 주소를 
	...
	const onWriterChangeHandle = (e) => {
		...
		// b_name = b_writer_text : 절대쓰면 안되는 코드
		setB_Name(b_writer_text); // 이렇게 사용해야 함
	};
	/**
	 * bbsVO 상태(객체)를 생성하고
	...
	const onChangeHandle = (e) =>{
		 ...
		 * name 변수에 담긴 값과 같은 요소를 value값으로 변경하여
		 * 원래 bbsVO와 교체하라
		 * 
