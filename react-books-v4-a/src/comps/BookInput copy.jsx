...
function BookInput() {
	 ...
	 * 		데이터(state)가 변경 되었을대만 reRendering 된다
	 * 변수는 무조건 초기화 되어 버린다
	 * 
	 * 초기화 되는 것을 방지하면서 현재 화면에서
	 * 어떤 변수(public 변수)의 값을 유지하고 싶을때
	 * 
	...
	// 도서정보를 배열에 저장하는데
	// 타이밍에 따라 아직 UUID가 미쳐 추가되지 않은 상태에서
	// 리스트에 추가되는 것을 방지하기 위하여 Promise 방식으로
	// 함수를 실행한다
	// 바깥쪽 함수에 async 키워드 추가
	// 내부에서 순서를 지켜야할 함수들에 await 추가
	const bookInsert = async () =>{
		 ...
		 * 입력창에 입력된 내용을 삭제하기
		 * 이 기능을 사용하려면
		 * 반드시 input box에 value를 사용해야 한다
		 * <input value={state} />
		 * 
		 * <input defaultValue={stata} />를 사용하면
		 * 기능이 효과 없음
		 */
